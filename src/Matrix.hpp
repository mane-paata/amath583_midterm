...
public:
  ColMatrix(size_t M, size_t N) : num_rows_(M), num_cols_(N), storage_(num_rows_ * num_cols_) {}
        ...
        double& operator()(size_t i, size_t j)       { return storage_[i * num_cols_ + j]; /* FIXME */}
  const double& operator()(size_t i, size_t j) const { return storage_[i * num_cols_ + j]; /* FIXME */}
